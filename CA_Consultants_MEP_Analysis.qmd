...
@media print {
  ...
  .progress-bar-container, .back-to-top { display: none !important; }
}
</style>
```
...
required_pkgs <- c(
  ...
  "viridis", "effectsize", "crosstalk", "htmltools"
)
...
cat(sprintf("**Kruskal-Wallis Test (primary — non-parametric):**\n\n- χ² = %.2f, df = %d, **p-value = %.4f**\n- Effect size (η²[H]): %.3f (%s)\n\n**Welch's ANOVA (robustness check — does not require equal variances):**\n\n- F(%.1f, %.1f) = %.2f, **p-value = %.4f**\n- Both tests agree: %s\n",
            ...
            if_else(kw_test$p.value < 0.05 & welch_test$p.value < 0.05,
...
**[https://github.com/YOUR-USERNAME/ca-consultants-mep-analytics](https://github.com/YOUR-USERNAME/ca-consultants-mep-analytics)**
...
> **Setup instructions (do this before submission):**
>
> 1. Go to [github.com](https://github.com) and create a new **public** repository named `ca-consultants-mep-analytics`
> 2. Upload `CA_Consultants_MEP_Analysis.qmd` and `mep_projects.csv`
> 3. Add a short `README.md` (GitHub will prompt you)
> 4. Replace `YOUR-USERNAME` above with your actual GitHub username
> 5. Paste the repo URL into your LMS submission alongside the RPubs link
